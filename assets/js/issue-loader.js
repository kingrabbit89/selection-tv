...
fetch(jsonUrl,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('HTTP '+r.status);return r.json()}).then(async d=>{
 ...
 await addCss(root+'assets/css/'+(d.theme==='magazine'?'magazine.css?v=20260924-radarfix2':'archive.css?v=20260924-radarfix2'));
 ...
 await addScript(root+'assets/js/'+(d.theme==='magazine'?'magazine-week.js?v=20260924-radarfix2':'archive-week.js?v=20260924-radarfix2'));
 if(d.theme==='magazine'){
   try{
   ...
   await addScript(root+'assets/js/seen-filter.js?v=20260924-radarfix2');
 }
 await addScript(root+'assets/js/page-layout.js?v=20260924-radarfix2');
