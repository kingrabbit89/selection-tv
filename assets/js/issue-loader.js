...
fetch(jsonUrl,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('HTTP '+r.status);return r.json()}).then(async d=>{
 ...
 await addCss(root+'assets/css/'+(d.theme==='magazine'?'magazine.css?v=20260924-radarreserve4':'archive.css?v=20260924-radarreserve4'));
 ...
 await addScript(root+'assets/js/'+(d.theme==='magazine'?'magazine-week.js?v=20260924-radarreserve4':'archive-week.js?v=20260924-radarreserve4'));
 if(d.theme==='magazine'){
   await addScript(root+'assets/js/seen-filter.js?v=20260924-radarreserve4');
   try{const rr=await fetch(root+'data/radar-reserves/'+week+'.json',{cache:'no-store'});if(rr.ok){const rd=await rr.json();const {schema_version:_schema,week:_week,...reserves}=rd;window.SELECTION_TV_RADAR_RESERVES=reserves}}catch(e){}
   await addScript(root+'assets/js/radar-reserve.js?v=20260924-radarreserve4');
 }
 await addScript(root+'assets/js/page-layout.js?v=20260924-radarreserve4');
