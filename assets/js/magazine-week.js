...
document.querySelectorAll('article.week-card,article.list-card,article.platform,article.feature,article.radar-card,table.schedule tbody tr').forEach(el=>{
 ...
 el.querySelectorAll('.program-actions a').forEach(a=>{
   ...
   let u=null;
   if(label==='imdb') u=d.imdb;
   else if(label==='senscritique') u=d.sc;
   else if(label==='wikipedia') u=d.wiki;
   else if(label==='allociné') u=d.allocine;
   else if(label==='fiche') u=pref(d);
   else if(label.startsWith('voir sur')) u=d.official;
   if(u) a.href=u; else a.remove();
 });
 el.querySelectorAll('.ratings a').forEach(a=>{
   const label=clean(a.textContent).toLowerCase();
   const u=label.startsWith('imdb')?d.imdb:label.startsWith('senscritique')?d.sc:null;
   if(u) a.href=u; else a.remove();
 });
});
...
document.querySelectorAll('article.radar-card').forEach(card=>{
 if(card.dataset.saveAdded==='1') return;
 card.dataset.saveAdded='1';
 const title=(card.querySelector('h3')?.textContent||'').trim();
 if(!title) return;
 const key=norm(title);
 const added=(card.querySelector('.added')?.textContent||'Radar 1080p').trim();
 ...
 b.onclick=()=>{
   ...
   else all[key]={title,context:'Radar 1080p · '+added,badge:'RADAR 1080P',page:document.title,url:location.href,added:new Date().toISOString(),status:'a-recuperer'};
   save(all);refresh();
 };
